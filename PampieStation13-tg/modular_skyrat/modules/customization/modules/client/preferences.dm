		...
		if (0) // Character Settings#
			...
			switch(character_settings_tab)
				if(0) //General
					...
					dat += "<br><br><b>Accident Types:</b><BR>"

					dat += "<a href='?_src_=prefs;preference=accident_types;task=input'>[accident_types]</a><BR>"
	...
	switch(href_list["task"])
		...
		if("input")
			...
			switch(href_list["preference"])
				...
				if("accident_types")
					var/accident_types2 = input(user, "What accidents does this character have?", "Character Preference", accident_types) as null|anything in sortList(list("Pee Only","Poop Only","Both"))
					if (accident_types2)
						accident_types = accident_types2
